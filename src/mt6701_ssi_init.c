...
/**
 * @brief 初始化MT6701 SSI接口引脚
 * 
 */
static inline void mt6701_ssi_init_pins()
{
...
int mt6701_ssi_init(uint32_t baud, sei_trigger_input_config_t *trigger_config, void (*isr_callback)(uint32_t))
{
    ...
    uint16_t ck1_point = sei_get_xcvr_ck1_point(BOARD_MTSEI, BOARD_MTSEI_CTRL);
    // 设置数据接收点时刻(设置为时钟下降沿)
    sei_set_xcvr_rx_point(BOARD_MTSEI, BOARD_MTSEI_CTRL, ck0_point);
    // 设置数据发送点时刻(设置为时钟上升沿)
    sei_set_xcvr_tx_point(BOARD_MTSEI, BOARD_MTSEI_CTRL, ck1_point);
    ...
    sei_cmd_data_format_config_init(BOARD_MTSEI, SEI_SELECT_DATA, SEI_DAT_2, &data_format_config);
    /* 数据寄存器2保存4bit状态 */
    ...
    sei_cmd_data_format_config_init(BOARD_MTSEI, SEI_SELECT_DATA, SEI_DAT_3, &data_format_config);
    /* 数据寄存器3保存6bit CRC */
    data_format_config.mode = sei_data_mode;
    data_format_config.signed_flag = false;
    data_format_config.bit_order = sei_bit_msb_first;
    ...
    data_format_config.min_bit = 0;
    sei_cmd_data_format_config_init(BOARD_MTSEI, SEI_SELECT_DATA, SEI_DAT_4, &data_format_config);
    int index = 0;
    /* [3] sei 指令 */
    ...
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_0, SEI_CTRL_LATCH_TRAN_0_1,
    ...
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_0, SEI_CTRL_LATCH_TRAN_1_2,
                                     &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true; // 全不检查则为空状态跳过
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_0, SEI_CTRL_LATCH_TRAN_2_3,
                                     &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    ...
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_0, SEI_CTRL_LATCH_TRAN_3_0,
                                     &state_transition_config);
    ...
    sei_state_transition_latch_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_0, &state_transition_latch_config);
    ...
    state_transition_config.instr_ptr_cfg = 3;               // 0-3：匹配/不匹配/开始/[结束]
    state_transition_config.instr_ptr_value = 4;             // CRC传输完成即视为数据接收完成
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_0_1,
                                     &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true; // 全不检查则为空状态跳过
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_1_2,
                                     &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true; // 全不检查则为空状态跳过
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_2_3,
                                     &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true; // 全不检查则为空状态跳过
    sei_state_transition_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_3_0,
                                     &state_transition_config);

    state_transition_latch_config.enable = true;                           // 启用状态机
    state_transition_latch_config.output_select = SEI_CTRL_LATCH_TRAN_0_1; // 状态0-1时输出(传输完成)
    state_transition_latch_config.delay = 0;                               // 延时0
    sei_state_transition_latch_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, SEI_LATCH_1, &state_transition_latch_config);

    /* [5] 时间戳采样配置 */
    sei_sample_config_t sample_config = {0};
    sample_config.latch_select = SEI_LATCH_0; // 状态机0
    sei_sample_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, &sample_config);
    ...
    sei_update_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, &update_config);

    // 状态机1事件和错误事件中断
    sei_set_irq_enable(BOARD_MTSEI, BOARD_MTSEI_CTRL, sei_irq_latch1_event | sei_irq_trx_err_event, true);
    ...
    sei_engine_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, &engine_config);
    sei_set_engine_enable(BOARD_MTSEI, BOARD_MTSEI_CTRL, true);

    /* [10] 触发配置 */
    sei_trigger_input_config_init(BOARD_MTSEI, BOARD_MTSEI_CTRL, trigger_config);
 ...
 * @brief 中断服务函数
 */
static SDK_DECLARE_EXT_ISR_M(BOARD_MTSEI_IRQn, __isr_sei_fun)
static void __isr_sei_fun(void)
{
    if (sei_get_irq_status(BOARD_MTSEI, BOARD_MTSEI_CTRL, sei_irq_latch1_event))
    {
        sei_clear_irq_flag(BOARD_MTSEI, BOARD_MTSEI_CTRL, sei_irq_latch1_event);
        __isr_callback(sei_irq_latch1_event);
    }

    if (sei_get_irq_status(BOARD_MTSEI, BOARD_MTSEI_CTRL, sei_irq_trx_err_event))
    {
        sei_clear_irq_flag(BOARD_MTSEI, BOARD_MTSEI_CTRL, sei_irq_trx_err_event);
